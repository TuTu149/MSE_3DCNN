...
def SVM(data):
	...
	x_train1, x_test1, y_train1, y_test1 = train_test_split(x1, y1, random_state=1, train_size=0.01)

	x2, y2 = np.delete(X2, dim, axis=1), X2[:, dim]
	x_train2, x_test2, y_train2, y_test2 = train_test_split(x2, y2, random_state=1, train_size=0.01)

	x3, y3 = np.delete(X3, dim, axis=1), X3[:, dim]
	x_train3, x_test3, y_train3, y_test3 = train_test_split(x3, y3, random_state=1, train_size=0.01)

	x4, y4 = np.delete(X4, dim, axis=1), X4[:, dim]
	x_train4, x_test4, y_train4, y_test4 = train_test_split(x4, y4, random_state=1, train_size=0.01)

	x5, y5 = np.delete(X5, dim, axis=1), X5[:, dim]
	x_train5, x_test5, y_train5, y_test5 = train_test_split(x5, y5, random_state=1, train_size=0.01)

	x6, y6 = np.delete(X6, dim, axis=1), X6[:, dim]
	x_train6, x_test6, y_train6, y_test6 = train_test_split(x6, y6, random_state=1, train_size=0.01)

	x7, y7 = np.delete(X7, dim, axis=1), X7[:, dim]
	x_train7, x_test7, y_train7, y_test7 = train_test_split(x7, y7, random_state=1, train_size=0.01)

	x8, y8 = np.delete(X8, dim, axis=1), X8[:, dim]
	x_train8, x_test8, y_train8, y_test8 = train_test_split(x8, y8, random_state=1, train_size=0.01)

	x9, y9 = np.delete(X9, dim, axis=1), X9[:, dim]
	x_train9, x_test9, y_train9, y_test9 = train_test_split(x9, y9, random_state=1, train_size=0.01)
	...
	x_test = np.vstack((x_test1, x_test2, x_test3, x_test4, x_test5, x_test6, x_test7, x_test8, x_test9))
	y_test = np.hstack((y_test1, y_test2, y_test3, y_test4, y_test5, y_test6, y_test7, y_test8, y_test9))
...
data1=excel2m('D:/pyproject/PU数据集/win9.csv')
table=excel2m('D:/pyproject/PU数据集/label.csv')
data = np.column_stack((data1, table))
x_train, y_train, x_test, y_test=SVM(data)
x_test = data[:, 0:8343]
y_test = data[:, 8343:8344]#将y_test重新赋值
...
xtrain = np.ndarray((x_train.shape[0], 8343, 3)) # 这里的(10000, 4096, 1)是ndarray的形状，随机初始化
xtest = np.ndarray((x_test.shape[0], 8343, 3))
...
xtrain = xtrain.reshape(x_train.shape[0], 9, 9, 103, 3)#(10000, 16, 16, 16, 1)
print("###",xtrain.shape)
xtest = xtest.reshape(x_test.shape[0], 9, 9, 103, 3)
