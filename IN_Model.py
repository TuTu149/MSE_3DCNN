...
def SVM(data):
	...
	x_train1, x_test1, y_train1, y_test1 = train_test_split(x1, y1, random_state=1, train_size=0.05)

	x2, y2 = np.delete(X2, dim, axis=1), X2[:, dim]
	x_train2, x_test2, y_train2, y_test2 = train_test_split(x2, y2, random_state=1, train_size=0.05)

	x3, y3 = np.delete(X3, dim, axis=1), X3[:, dim]
	x_train3, x_test3, y_train3, y_test3 = train_test_split(x3, y3, random_state=1, train_size=0.05)

	x4, y4 = np.delete(X4, dim, axis=1), X4[:, dim]
	x_train4, x_test4, y_train4, y_test4 = train_test_split(x4, y4, random_state=1, train_size=0.05)

	x5, y5 = np.delete(X5, dim, axis=1), X5[:, dim]
	x_train5, x_test5, y_train5, y_test5 = train_test_split(x5, y5, random_state=1, train_size=0.05)

	x6, y6 = np.delete(X6, dim, axis=1), X6[:, dim]
	x_train6, x_test6, y_train6, y_test6 = train_test_split(x6, y6, random_state=1, train_size=0.05)

	x7, y7 = np.delete(X7, dim, axis=1), X7[:, dim]
	x_train7, x_test7, y_train7, y_test7 = train_test_split(x7, y7, random_state=1, train_size=0.05)

	x8, y8 = np.delete(X8, dim, axis=1), X8[:, dim]
	x_train8, x_test8, y_train8, y_test8 = train_test_split(x8, y8, random_state=1, train_size=0.05)

	x9, y9 = np.delete(X9, dim, axis=1), X9[:, dim]
	x_train9, x_test9, y_train9, y_test9 = train_test_split(x9, y9, random_state=1, train_size=0.05)

	x10, y10 = np.delete(X10, dim, axis=1), X10[:, dim]
	x_train10, x_test10, y_train10, y_test10 = train_test_split(x10, y10, random_state=1, train_size=0.05)

	x11, y11 = np.delete(X11, dim, axis=1), X11[:, dim]
	x_train11, x_test11, y_train11, y_test11 = train_test_split(x11, y11, random_state=1, train_size=0.05)

	x12, y12 = np.delete(X12, dim, axis=1), X12[:, dim]
	x_train12, x_test12, y_train12, y_test12 = train_test_split(x12, y12, random_state=1, train_size=0.05)

	x13, y13 = np.delete(X13, dim, axis=1), X13[:, dim]
	x_train13, x_test13, y_train13, y_test13 = train_test_split(x13, y13, random_state=1, train_size=0.05)

	x14, y14 = np.delete(X14, dim, axis=1), X14[:, dim]
	x_train14, x_test14, y_train14, y_test14 = train_test_split(x14, y14, random_state=1, train_size=0.05)

	x15, y15 = np.delete(X15, dim, axis=1), X15[:, dim]
	x_train15, x_test15, y_train15, y_test15 = train_test_split(x15, y15, random_state=1, train_size=0.05)

	x16, y16 = np.delete(X16, dim, axis=1), X16[:, dim]
	x_train16, x_test16, y_train16, y_test16 = train_test_split(x16, y16, random_state=1, train_size=0.05)
...
xtrain = np.ndarray((x_train.shape[0], 24200, 3)) # 这里的(10000, 4096, 1)是ndarray的形状，随机初始化
xtest = np.ndarray((x_test.shape[0], 24200, 3))
...
xtrain = xtrain.reshape(x_train.shape[0], 11, 11, 200, 3)#(10000, 16, 16, 16, 1)
print("###",xtrain.shape)
xtest = xtest.reshape(x_test.shape[0], 11, 11, 200, 3)
...
input_layer0 = Input((11, 11, 200, 3))
