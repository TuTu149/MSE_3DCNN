...
data1=excel2m('D:/pyproject/KSC数据集/win11.csv')
table=excel2m('D:/pyproject/KSC数据集/label.csv')
data = np.column_stack((data1, table))
x_train, y_train, x_test, y_test=SVM(data)
x_test = data[:, 0:21296]
y_test = data[:, 21296:21297]#将y_test重新赋值
...
xtrain = np.ndarray((x_train.shape[0], 21296, 3)) # 这里的(10000, 4096, 1)是ndarray的形状，随机初始化
xtest = np.ndarray((x_test.shape[0], 21296, 3))
...
xtrain = xtrain.reshape(x_train.shape[0], 11, 11, 176, 3)#(10000, 16, 16, 16, 1)
print("###",xtrain.shape)
xtest = xtest.reshape(x_test.shape[0], 11, 11, 176, 3)
...
input_layer0 = Input((11, 11, 176, 3))
...
output_layer = Dense(units=13, activation='softmax')(dense_layer2)
